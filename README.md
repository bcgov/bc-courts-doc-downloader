...
A BC Court SCV application user initiates a request for the pre-download of an application record from either the Civil or Criminal court list.  

The DocDownloaderAPI receives the request and responds to the caller with an immediate 'transferId' while concurrently initiating a new job (thread) to carry out the following tasks:  
...
* At any time during processing, the application requesting the file transfer may request the file transfer status.    
